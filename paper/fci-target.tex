...
\section{Why the benchmark fails}
\label{sec:sim}

Table~\ref{tab:bench} is a pattern, not an explanation. This section supplies
one with a simulation calibrated to the paper's own design, in which the
predictor is informative at every horizon by construction, so that anything the
benchmark does wrong is the benchmark's doing.

The data generating process is
\begin{equation}
x_t = \phi x_{t-1} + \eta_t, \qquad
u_t = \rho u_{t-1} + \theta x_{t-1} + \varepsilon_t,
\end{equation}
with $\phi = 0.8$, $\theta = 0.3$, and $\eta_t, \varepsilon_t$ standard normal
and independent. The estimation sample is $T = 80$ quarters, the length of the
panel's, and the four forecasts of the path target are formed exactly as in
Section~2. Results are over 4{,}000 replications.

Two things happen as $\rho$ approaches one. The estimated own-lag coefficient
falls further below its population value, and the shortfall grows with the
horizon: at $\rho = 0.99$ and $h = 8$ the population projection coefficient is
$0.973$ and the average estimate is $0.789$. And the cost of that shortfall,
measured against a random walk, rises --- Panel A of Table~\ref{tab:sim}. At
$\rho = 0.90$ the estimated autoregression is at worst four per cent behind a
random walk; at $\rho = 0.99$ it is twenty-two per cent behind at two years.
This is the pattern of Table~\ref{tab:bench}, and the parameter that drives it
is the persistence of the target.

Panel B shows the consequence for the predictor. Against the estimated
autoregression the index improves forecasts by between eight and nineteen per
cent everywhere, at every persistence and every horizon, exactly as it does in
the data. Against the best of the three benchmarks that advantage survives at
short horizons and disappears at two years once $\rho$ reaches $0.97$. Nothing
about the predictor changes across the columns of Panel B; only the benchmark
does.

\begin{table}[htbp]
\centering
\caption{Simulation: persistence, the benchmark, and the apparent value of a predictor}
\label{tab:sim}
\begin{tabular}{lrrrrcrrrr}
\toprule
& \multicolumn{4}{c}{Panel A: AR $\div$ random walk} & &
  \multicolumn{4}{c}{Panel B: index $\div$ best benchmark} \\
\cmidrule(lr){2-5} \cmidrule(lr){7-10}
$\rho$ & $h{=}1$ & $h{=}2$ & $h{=}4$ & $h{=}8$ & & $h{=}1$ & $h{=}2$ & $h{=}4$ & $h{=}8$ \\
\midrule
0.90 & 1.011 & 1.009 & 1.024 & 1.043 & & 0.918 & 0.882 & 0.873 & 0.905 \\
0.95 & 1.015 & 1.036 & 1.072 & 1.151 & & 0.907 & 0.906 & 0.882 & 0.950 \\
0.97 & 1.023 & 1.041 & 1.082 & 1.173 & & 0.922 & 0.894 & 0.904 & 0.992 \\
0.99 & 1.027 & 1.037 & 1.098 & 1.224 & & 0.922 & 0.906 & 0.893 & 1.008 \\
1.00 & 1.015 & 1.039 & 1.087 & 1.216 & & 0.926 & 0.889 & 0.891 & 1.005 \\
\bottomrule
\end{tabular}

\vspace{0.5em}
\begin{minipage}{0.95\textwidth}
\footnotesize \textit{Notes.} 4{,}000 replications, $T = 80$, path target. The
predictor enters the data generating process with $\theta = 0.3$ and therefore
carries genuine predictive content at every horizon and every $\rho$. Panel A
reports the root mean squared error of the estimated own-lag regression divided
by that of a random walk; Panel B the index regression divided by the best of
the recursive mean, the random walk and the own-lag regression.
\end{minipage}
\end{table}

The simulation understates what the panel shows --- its two-year ratios reach
$1.008$ where the data reach $1.316$ --- which is expected, since a stationary
first-order process with a constant coefficient is a generous description of an
unemployment rate over 2006--2025. The point is the comparative static rather
than the level: the more persistent the target, the more an estimated
autoregression flatters whatever is added to it, and the less that flattery
means.
